...
const WorkoutLogs = () => {
    const { workoutHistory, deleteWorkoutLog, updateWorkoutLog, addWorkoutLog } = useProgressStore();
    const { addXp } = useAuthStore();
    const [selectedLogId, setSelectedLogId] = useState(null);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    
    // Fallback history array
    const activeHistory = workoutHistory && workoutHistory.length > 0 ? workoutHistory : [
    ...
    // Stats
    const totalDuration = activeHistory.reduce((sum, item) => sum + Number(item.duration), 0);
    const totalCalories = activeHistory.reduce((sum, item) => sum + Number(item.calories), 0);
    const avgFatigue = activeHistory.length > 0 ? 'Optimal Recovery' : 'No Data';
                                ...
                                <div key={log.id} className="relative overflow-hidden border border-slate-800 bg-slate-950/40 rounded-xl p-5 hover:border-slate-700/50 transition-all">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                                        ...
                                        <div className="flex items-center gap-3 self-start sm:self-auto">
                                            <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full border ${fatigueColors[log.fatigue] || fatigueColors.medium}`}>
                                                {log.fatigue} fatigue
                                            </span>
                                            <button 
                                    ...
                                    <div className="grid grid-cols-2 gap-4 sm:flex sm:gap-6 border-b border-slate-900 pb-4 mb-4">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-sm font-semibold text-slate-300">{log.duration} mins</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Flame className="w-4 h-4 text-slate-500" />
                                            <span className="text-sm font-semibold text-slate-300">{log.calories} kcal</span>
                                        </div>
                                    </div>
                                        ...
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            {!isEditing ? (
                                                ...
                                                <button 
                                                    ...
                                                    className="text-xs font-bold text-neon-blue flex items-center gap-1 hover:underline focus:outline-none"
                                                >
